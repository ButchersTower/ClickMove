...
public class Npc {
	...
	// You can not walk into wall 7 from the right.
	// You can not walk out of wall 7 when going right.

	static int num = 342;

	static int fullHp = 200;
	static int health = fullHp;

	static float xx = 1296;
	static float yy = 510;
	...
	public static void drawHealth(Graphics g) {
		if (health > 0) {
			...
			g.drawImage(ImgLoad.health[de], (int) xx ,
					(int) yy  + 32, null);
	...
	public static int overlapOLD(float x, float y, int w, int h, double s, int d) {
